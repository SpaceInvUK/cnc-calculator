# FAST CNC Prototypes

These files are separate prototypes and are not the official calculator 1.0.

Official calculator 1.0 stays at:

- `../Cnc Calculator UI Test.html`
- `../CNC Calculator 1.0.html`

## New Design

Local file:

```text
C:\Users\ednei\Documents\CNC App\prototypes\new-design\index.html
```

GitHub Pages:

```text
https://spaceinvuk.github.io/cnc-calculator/prototypes/new-design/
```

## Order Entry

Local file:

```text
C:\Users\ednei\Documents\CNC App\prototypes\order-entry\index.html
```

GitHub Pages:

```text
https://spaceinvuk.github.io/cnc-calculator/prototypes/order-entry/
```

Related beta file:

```text
C:\Users\ednei\Documents\CNC App\prototypes\order-entry\beta.html
```

Beta GitHub Pages:

```text
https://spaceinvuk.github.io/cnc-calculator/prototypes/order-entry/beta.html
```
